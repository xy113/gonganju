...
function Dialog(content, options)
{
	...
	var dialogContent = '<div id="' + options.id + '" class="dialog">'+barHtml+'<div class="content"></div></div>';
	var document = window.top.document;
	$(document).find('body').append(dialogContent);
	var dialog = $(document).find("#"+options.id );
	//$('body').append(dialog);
	dialog.hide();
    /**
     * 重置对话框的位置。
    ...
    var resetPos = function()
    {
        ...
        if(options.center)
        {
            var left = ($(window.top).width() - dialog.width()) / 2;
            var top = ($(window.top).height() - dialog.height()) / 2-100;
    ...
    var init = function()
    {
        ...
        {
			if(isIe6){
            	$(document).find('body').append('<div id="' + overlayId + '" class="dialog-overlay"><iframe frameborder="0" name="mainframe" id="mainframe" style="width:100%; height:100%; FILTER: alpha(opacity=0); opacity:0"></iframe></div>');
			}else{
				$(document).find('body').append('<div id="' + overlayId + '" class="dialog-overlay"></div>');
			}
			$(document).find('#' + overlayId).css({'left':0, 'top':0,
		...
		/*  IE6 兼容fixed代码 */
		if(isIe6){dialog.css('width','384px');}
        if(isIe6 && options.fixed)
        {
    ...
    this.setContent = function(c)
    {
        ...
        if('object' == typeof(c))
        {
            ...
            case 'url':
                ...
                break;
            case 'iframe':
                div.append($('<iframe frameborder="0" src="' + c.value + '"></iframe>'));
                break;
            case 'text':
            default:
                div.html(c.value);
    ...
    this.show = function()
    {
        ...
        {
            if(!isIe){return $(document).find('#' + id).css('opacity'); }
        ...
        /* 是否显示背景遮罩层 */
        if(options.modal)
        {   $(document).find('#' + overlayId).fadeTo('fast', getOpacity(overlayId));   }
    ...
    this.hide = function()
    {
        ...
        if(options.modal)
        {   $(window.parent.document).find('#' + overlayId).fadeOut('fast');   }
    ...
    this.close = function()
    {
        ...
        if(options.modal)
        {   $(window.parent.document).find('#'+overlayId).fadeOut('fast', function(){$(this).remove();}); }
        clearTimeout(timeId);
    }
    init.call(this);
