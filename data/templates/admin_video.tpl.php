...
<option value="0" class="bold"><?php echo $lang['video_all']?></option><? if(is_array($categories['category'])) { foreach($categories['category'] as $cat) { ?><option value="<?php echo $cat['cid']?>" class="bold"<? if($cid==$cat['cid']) { ?> selected="selected"<? } ?>><?php echo $cat['name']?></option><? if(is_array($categories['sub'])) { foreach($categories['sub'] as $sub) { if($sub['fid']==$cat['cid']) { ?>
...
<div class="wrapper">
  ...
  <th width="50"><?php echo $lang['recommend']?></th>
    ...
    </tr><? if(is_array($videos)) { foreach($videos as $video) { ?>    <tr onmouseOver="$(this).addClass('hover')" onmouseOut="$(this).removeClass('hover')">

      <td><input type="checkbox" class="checkbox" name="id[]" value="<?php echo $video['id']?>"></td>

      <td><a href="play.php?id=<?php echo $video['id']?>" target="_blank"><?php echo $video['title']?></a></td>
  ...
  <td><img src="/static/images/<? if($video['recommend']) { ?>yes.gif<? } else { ?>no.gif<? } ?>" border="0" /></td>

      <td><img src="/static/images/<? if($video['audited']) { ?>yes.gif<? } else { ?>no.gif<? } ?>" border="0" /></td>

      <td><?php echo $video['pubtime']?></td>

      <td><a href="<?php echo $BASESCRIPT?>?action=<?php echo $action?>&operation=edit&id=<?php echo $video['id']?>"><?php echo $lang['edit']?></a></td>
